...
```
https://gitee.com/daiyuxiang520/geren-update/releases/download/v82/wuling-assistant.apk
```
...
| 版本 | 说明 |
|---|---|
| **v82** (3.79.0) | **能耗趋势时间轴修复**：X 轴标签改用锚点绘制+自适应抽稀，日维度不再出现被裁掉的「0 0 1 1 2 2 3」乱码；月/年维度趋势图改为画「所选年份」的 1–12 月，年维度切到 2025 即显示 2025-01~2025-12，与汇总卡片一致；TrendPoint 携带真实日期，弹窗与明细不再靠 now() 反推；日维度进入即高亮所选日；柱状图与油耗折线图 X 坐标统一对齐。沿用 v81 蓝牙自动连接修复。MD5 `72e629c842e93e38a21d562cfeb4f866` |
| **v81** (3.78.0) | **蓝牙自动连接不再等联网**：冷启动 BLE 与网络刷新并行执行（旧实现排在最多 3 次联网刷新之后，地库无信号时联网请求挂起会把蓝牙一起拖住，表现为「从不自动连接」）；新增「回到前台」「蓝牙被打开」两个补连触发点；本地没有蓝牙钥匙时自动向服务器获取后再连；「蓝牙自动连接」开关即时生效（打开即连、关闭即停）；手动断开后 10 分钟内不被回前台补连拉回。沿用 v80 NFC 空白标签可写入。MD5 `81edc2258d5f5afa86f184c3c6915b0a` |
