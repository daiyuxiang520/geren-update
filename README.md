...
| 版本 | 说明 |
|---|---|
| **v36** (3.33.0) | **修复「天气已取到但界面空白」**：车辆刷新时坐标短暂为 null 会重启解析协程并清空数据，现用「最后有效坐标」兜底；单次网络失败时天气用上次结果兜底，不再闪空 |
| **v35** (3.32.0) | **天气双保险**：高德优先，未配置/未开通服务/失败时**自动降级 Open-Meteo**（无需 Key，经纬度直查）→ 只要有坐标就有天气；天气行标注数据来源；高德原始错误码接入「我的 → 调试日志」 |
| **v34** (3.31.0) | **修复「检测不到新版本」**：检查更新改为**并发请求全部加速源、取版本号最大**的结果（原来「首个成功即用」，靠前的源若缓存旧版就会永远提示已是最新）；有新版本时提前返回更快 |
| **v33** (3.30.0) | **彻底修复「有地址没天气」**：地址与天气合并为单协程串行链路（不再互抢刷新）；天气失败**明确提示**原因（Key 无效/未绑服务/配额用尽/网络异常）；天气行新增更新时间 |
| **v32** (3.29.0) | **修复位置页看不到详细地址**：地址解析改用**高德逆地理编码**（原接口已失效）；未配置/无效 Key 时给出明确提示 |
| **v30** (3.27.0) | **完善开源致谢**：App 内「关于我们」页新增 **hasscc/wuling**（车控协议思路来源）署名，第三方组件清单补全 |

> 📜 **更早版本**（v31 及以前）请见 [**Releases 页面**](https://github.com/daiyuxiang520/geren-update/releases)。
